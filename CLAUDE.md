...
> 最后更新：2026-09-10｜页面代码版本 v3（version.json）｜内容版本 2（content.json，1-4 周已细化，48 周核心词句已补满）
...
| 页面代码 | v3：三页齐全（本周页 / 当天详情页 / 全年地图页）+ PWA 套件 + 更新徽标；返回键走历史记录、休息日无打卡键、本周页有「整周打卡」 |
| 测试 | 31/31 通过（content 7 + logic 10 + ui 8 + nav 6） |
...
| `version.json` | 页面代码发布版本 `{"version": 3}`，驱动「点我更新」徽标 |
...
| `tests/` | 测试：content / logic / ui / nav 四个文件共 31 条，`node --test` 跑（nav.test.js 用最小浏览器桩验证返回键与整周打卡） |
| `tools/` | `make-icons.py`：Pillow 脚本；`python tools/make-icons.py` 出正式图标，`python tools/make-icons.py options` 出三方案对比图 `docs/icon-options.png`。改图标后要提交新的 PNG |
...
1. **测试**：`node --test`，31 条全绿才算过。注意参数不带 `tests/`（Windows 本机带目录参数会报错）。若本机沙箱让 `node --test` 报 `spawn EPERM`（它要开子进程管道），就逐个跑 `node tests/content.test.js`、`node tests/logic.test.js`、`node tests/ui.test.js`、`node tests/nav.test.js`，同样一份测试、同样 31 条。
...
- **测试**：`tests/` 下四个文件 31 条，直接读 content.json 与 require logic.js/ui.js，不开浏览器；`nav.test.js` 自建最小 window/document/history 桩来验证返回键与整周打卡行为。
