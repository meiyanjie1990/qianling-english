...
- **48 周的核心词 / 核心句都已补满并升级**（2026-09-15，内容版本 5）：句子全部换成 native 妈妈日常话、每周期加 2-3 个新词、难度按周爬坡；非细化周一律 ≥5 个核心词、≥2 个核心句，不再有空数组。
- 目前第 1-24 周已细化（到 12 月中的「综合复习」）；第 25 周起只有骨架，App 内显示「内容还没出」。
- **每周的详细计划另有一份 md**，在 `E:\谦灵英语启蒙\第NN周-主题-English.md`（每周一个文件，第 01-24 周齐全）。md 是给 Mei 看的完整教案（视频/逐日计划/冰箱贴/配套讲义/进阶/提醒），content.json 是 App 读的数据版——**同一份内容两个出口**。改内容的顺序是：JSON 片段 → `merge-weeks.js` 合并 → `node tools/gen-md.js` 反向生成 md（md 不手改）。
- 打开 App 停在**上一次打卡的那一周**（v5 起；v6 补了老数据迁移，更新后第一次打开就正确）。从没打过卡才退回上次存的周号，再不行才用默认周 `DEFAULT_WEEK=2`。
...
- **写新周的工作目录：`E:\AI项目\谦灵启蒙-每周计划\`**（2026-09-10 建）。里面是「怎么写一周新内容」的活教材，别删：
  - `_写作规范.md` —— 模板结构、JSON 规则、内容风格、自检清单
  - `第05周-家人-Family.md` + `week-05.json` —— 标杆样本（一手一数据，v5 升级版）
  - `_校验.js` —— `node "_校验.js"` 批量体检本目录的 md + JSON 片段
  - `第01-24周*.md` + `week-*.json` —— 本轮原件（内容与 `E:\谦灵英语启蒙\` 的 md、App 的 content.json 一致，留档用）
...
| 细化内容（details） | **第 1-24 周**，每周 7 天（休息日固定第 3、6、7 天，活动日只有 1/2/4/5），每周带主视频+进阶视频+进阶说明；**v5 逐日英文原句全部重写**（native 口语、称呼统一 Mummy/Daddy、原进阶词上移核心词） |
...
| `content.json` | 全部课程数据：48 周骨架（核心词句 v5 升级版）+ 细化周的逐日内容；顶部带内容版本号 `version` |
| `version.json` | 页面代码发布版本 `{"version": 7}`，驱动「点我更新」徽标 |
| `manifest.json` | PWA 名称「谦灵启蒙」、图标、主题色 #F6F1E6（燕麦纸底） |
| `sw.js` | Service Worker（浏览器后台脚本，管缓存）：content/version 走网络优先，其余缓存优先且**取到即存**（字体子集离线可用）；`CACHE_NAME` 即缓存版本 |
...
1. **写本周的 JSON 片段** `week-NN.json`（内容以 JSON 为准，md 是生成品）：顶层正好 video / advanced / days 三个键，`days` 恰好 7 项，休息日固定第 3、6、7 天，活动日（1/2/4/5）每天 ≥3 个时间段块。
   内容风格照 `E:\AI项目\谦灵启蒙-每周计划\_写作规范.md`（**含 2026-09 升级规范**：native 句型、难度按周爬坡、每周期核心词 +2-3 个、称呼 Mummy/Daddy）；标杆样本是同目录的 `第05周-家人-Family.md` + `week-05.json`（已是 v5 升级版）。
2. **合并进 content.json**（别手工改 JSON，排版会乱）：
...
3. **生成教案 md**：`node tools/gen-md.js` —— 从 content.json 反向生成 `E:\谦灵英语启蒙\` 的 24 份 md（不手改 md）；再把片段和 md 拷进 `E:\AI项目\谦灵启蒙-每周计划\` 留档。
4. **新内容出现生僻字时**：`node tools/fetch-fonts.js` 重抓字体子集（否则缺的字在手机上会掉成系统字体）。
5. 跑测试全绿（`node --test`；沙箱里报 EPERM 就逐个 `node tests/x.test.js`）。
6. **提交并 push**，线上验证 `curl` 返回 200。
7. 完成。手机端 content.json 走网络优先，下次打开**自动拉到新内容**——这一步**不用动 version.json / sw.js**，也不用让 Mei 点更新。

> 2026-09-10 起：Mei 明确**以后不再要 docx 三件套**，每周只要一个 md。第 1-4 周的三个 docx 已由 `第01周-…md` 等取代（遗留的 `第X周-主题\` 文件夹她自己删）。

## 发布规则

1. **测试**：`node --test`，37 条全绿才算过。注意参数不带 `tests/`（Windows 本机带目录参数会报错）。若本机沙箱让 `node --test` 报 `spawn EPERM`（它要开子进程管道），就逐个跑 `node tests/content.test.js`、`node tests/logic.test.js`、`node tests/ui.test.js`、`node tests/nav.test.js`，同样一份测试、同样 37 条。
...
- **打开停在哪一周**（v5/v6）：优先级 = `loadLastCheckinWeek`（`qianling-last-checkin-week`，打卡时写入）→ `inferLastCheckinWeek`（老数据迁移：打卡记录里"真有勾的周"里最大的那一周）→ `loadCurrentWeek` → `DEFAULT_WEEK`。这样 App 跟着她**真实进度**走，而不是跟着课表日期走（她可能比课表快或慢）。迁移那一层是为了让老手机**更新后第一次打开就落在正确的那一周**，不用等下一次打卡。
- **本地状态** localStorage 四键：`qianling-progress-v1`（打卡记录）、`qianling-current-week`（当前周）、`qianling-last-checkin-week`（上次打卡的周号）、`qianling-app-version`（已接受的版本号）。
- **测试**：`tests/` 下四个文件 37 条，直接读 content.json 与 require logic.js/ui.js，不开浏览器；`nav.test.js` 自建最小 window/document/history 桩，验证返回键、整周打卡、重开 App 回到上次打卡那周（`boot(store)` 可传同一份 localStorage 模拟重开）；`ui.test.js` 里有一条**拿真实 content.json 把每个细化周的每一天都渲染一遍**的回归测试（新加周把页面搞崩会当场报错）。
...
| 「出后面 N 周的详细计划」 | 走「每周新内容固定流程」：写 JSON 片段 → `node tools/merge-weeks.js <目录> 周号… --bump` → `node tools/gen-md.js` → 测试 → push |
| 「第 X 周的句子不顺口 / 想改几句」 | 内容单点修改：改对应 week 片段 → merge → gen-md → 测试 → push（纯内容更新，不动 version.json） |
...
| 「打开怎么没停在我现在这周」 | v5 起打开停在**上一次打卡的那一周**；没打过卡才用存过的周号。改了启动逻辑要跑 `tests/nav.test.js` 的「重开 App 会回到上一次打卡的那一周」 |
